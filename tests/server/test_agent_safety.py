...
def test_exec_tool_bad_level_uses_default(conn):
    result, intent = asyncio.run(
        _exec_tool("scan_host", {"target": "1.2.3.4", "level": "full"}, conn)
    )
...
def test_exec_tool_bad_scan_id_returns_error(conn):
    result, intent = asyncio.run(
        _exec_tool("get_findings", {"scan_id": "latest"}, conn)
    )
    data = json.loads(result)
...
def test_exec_tool_missing_target_returns_error(conn):
    result, intent = asyncio.run(_exec_tool("scan_host", {}, conn))
    data = json.loads(result)
    assert "error" in data
    assert intent is None
...
def test_exec_tool_unknown_tool_returns_error(conn):
    result, intent = asyncio.run(_exec_tool("delete_database", {}, conn))
    data = json.loads(result)
    assert "error" in data
    assert intent is None
...
def test_exec_tool_explain_cve_missing_id_returns_error(conn):
    result, intent = asyncio.run(_exec_tool("explain_cve", {}, conn))
    assert "error" in json.loads(result)


def test_exec_tool_get_history_bad_limit_uses_default(conn):
    result, intent = asyncio.run(
        _exec_tool("get_history", {"limit": "not_a_number"}, conn)
    )
    data = json.loads(result)
...
def test_system_prompt_is_not_rigid():
    ...
    # Old rigid report structure must be gone
...
def test_exec_tool_query_shodan_no_key():
    ...
    result, intent = asyncio.run(
        _exec_tool("query_shodan", {"ip": "1.1.1.1"}, conn, shodan_key=None)
    )
    data = json.loads(result)
    assert "error" in data
...
def test_exec_tool_unknown_returns_error():
    ...
    from aivas.tui.agent import _exec_tool
    conn = sqlite3.connect(":memory:")
    result, intent = asyncio.run(_exec_tool("nonexistent_tool", {}, conn))
    assert "error" in json.loads(result)
