...
def _build_risk_prompt(meta: dict, findings: list[dict]) -> str:
    ...
    if not findings:
        return (
            ...
            "Write a 3-bullet security brief. Format EXACTLY as three bullet points using '- ' prefix. "
            "No headers. No numbered lists.\n\n"
            "- Bullet 1 — Posture: State that the scan returned Grade A with zero vulnerabilities found. "
            "Explain what this means in plain language — the exposed services appear secure based on known CVEs.\n\n"
            "- Bullet 2 — Limitations: Note that a network scan has limits — it cannot see inside the machine, "
            ...
            "- Bullet 3 — Recommendation: Advise keeping software updated, rescanning periodically, "
    ...
    return (
        ...
        f"{top_line}\n\n"
        "Write an executive risk brief as exactly 3 bullet points using '- ' prefix. "
        "No headers. No numbered lists. No extra blank lines between bullets.\n\n"
        "- Bullet 1 — Posture: State the grade and what it means for the organization in plain language. "
        "Include the finding count by severity. Keep it factual and direct.\n\n"
        "- Bullet 2 — Primary Threat: Name the worst vulnerability — what software it affects, "
        ...
        "- Bullet 3 — Urgency and Action: Describe how urgent this is based on the severity above. "
...
def _build_remediation_prompt(meta: dict, findings: list[dict]) -> str:
    ...
    return (
        ...
        "Under each header, write one numbered list entry per software group. "
        "Each entry uses this EXACT inline format — all on one line after the number:\n\n"
        "  N. **Software** (CVEs) — **Installed:** X | **Fix:** Y | **Action:** Update via your OS package manager or download from <Vendor URL>\n\n"
        "Example of a correct entry:\n"
        "  1. **OpenSSH** (CVE-2024-6387, CVE-2023-38408) — **Installed:** 8.9p1 | **Fix:** 9.8p1 | **Action:** Update via your OS package manager or download from https://www.openssh.com/\n\n"
        "RULES:\n"
        "- NEVER split a single entry across multiple numbered lines. Software name, installed version, fix version, and action are all on ONE line.\n"
        "- NEVER output 'unknown' for Fix version. If the 'LOOK UP' marker appears, use your training knowledge to state the exact patched version (e.g. 'OpenSSH 9.8p1'). Only if truly uncertain, write 'latest stable release'.\n"
        "- Use the EXACT vendor URL from the 'Vendor:' field above — do not invent or omit it.\n"
        "- No shell commands (apt, dnf, etc.) — OS is unknown.\n"
        "- If [URGENT — actively exploited], prepend the entry with '⚠ URGENT: '.\n"
        "- For ungrouped CVEs (no software detected), write a brief one-line entry with the patched version and a generic action.\n"
